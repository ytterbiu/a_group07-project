...
### !BE note: List of outstanding jobs

- Review figure captions
- Format p-values
- Identify all statistical tests, list pre-requisites for the tests, then
  provide justification for our use case C
...
(further data cleaning is described in §\@ref(elementOne)). The historical price
of the index and its daily log-returns for 2018-2024 are shown in Figure
...
```{r initial-visualisation, fig.cap = "Nasdaq Composite price trend and daily volatility (2018–2024). The left figure shows the daily adjusted closing price. The right figure shows the daily log-returns which highlights day-to-day percentage changes and, subsequently, periods of high volatility.", echo = FALSE}
...
@Cont01022001[^cont2001quote]. The Anderson-Darling test was selected to test
the normality of these data as is it more sensitive to deviations in the tails
of the distribution.
...
(§\@ref(elementOneAdtest)) and provides further evidence against an assumption
...
# extract the subsamples as numeric vectors & drop NA
...
# get CI using Bias-Correct and Accelerated interval
...
kable(
  ...
  align = c("l", "l", "r", "r", "r"),
  escape = FALSE,
  caption = "Bootstrap BCa 95\\% confidence intervals for the sample variance $s^2$ in the six-month periods with minimum and maximum sample variance."
) %>%
  kable_styling(full_width = FALSE, position = "center")
...
The 95% BCa confidence intervals (CI) for the variance ($\sigma^2$) for each of
these two subsamples are shown in Table \@ref(tab:var-ci). There is no overlap
between the 95% CI for the 6-month ranges with the minimum and maximum variance
...
(§\@ref(elementFou)), and lack of independence (§\@ref(elementFiv)), our
analysis for §\@ref(elementThr) did not find statistical evidence to reject the
...
- element 1: Anderson-Darling test provides strong evidence that the $z_n$ data
  are not normally distributed
- element 2: bootstrapping with z-data and normal distribution also provides
  evidence against an assumption of normality
- element 3: Kruskal–Wallis test -\> we accept null hypothesis that measures of
  central tendancy of log-returns over six-month periods share the same
  distribution (/medians are the same?)
- element 4: 95% BCa confidence intervals do not overlap, providing statistical
  evidence that (true) variance is not the same in the minimum and maximum
  six-month periods of sample variable evaluated
- element 5: Chi-squared test provided evidence that $z_n$ and $z_{n-1}$ are not
  indendent. BUT haven't checked for $y_{n-1}$ as not sure what this is...
- element 6:
  - Part 1: Binomial test revealed proportion of positive-return days is
    statistically greater than 50% (insert p \<0.001), confirming a general
    upward bias.
  - Part 2: Wilcoxon rank sum test with continuity correction provided
    sufficient statistical evidence (p\<0.001) to reject null hypothesis in
    favour of alternative hypothesis that the positive runs have a greater
    mean/median/distribution that the negative runs.
...
In \@ref(elementThr) the log-returns data were split into six-month periods, as
shown in Figure \@ref(fig:element3-boxplot).

![](fig/element3-boxplot-1.pdf)
...
[^log-symmetry]:
    With simple percentages, a 100% gain (+100%) is not cancelled out by a 100%
    loss (−100%). It's cancelled out by a 50% loss (−50%). This asymmetry makes
    statistical modeling difficult. With log-returns, a move from $100 to $200
    (a log-return of +0.693) is the exact mathematical opposite of a move from
    $200 to $100 (a log-return of −0.693).
