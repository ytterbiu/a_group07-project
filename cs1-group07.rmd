...
# report.
# We prepared the data by getting the adjusted close price, applying a log
...
The Nasdaq Composite Index (Nasdaq) is a stock market index that includes
thousands of stocks listed on the Nasdaq exchange (!ref). It is heavily focused
on technology companies, widely used as a key benchmark for the tech sector's
performance, and the index’s behaviour can impact many financial products,
including index-tracking funds (!refs).

In the standard asset-pricing model that underpins much of mathematical finance,
the log-price of an asset evolves according to Equation \@ref(eq:zn),
...
where the log-returns $Z_n$ are assumed to be normally distributed with constant
mean $\mu$, variance $\sigma^{2}$, and are independent over time. This project
uses the Nasdaq as a case study to assess how well these four assumptions
(normality, constant mean, constant variance, and independence of daily
log-returns) hold in practice.

The adjusted Nasdaq closing price was analysed because it accounts for corporate
actions like dividends and stock splits, giving a more accurate picture of an
investor’s actual returns. Daily log-returns were calculated from the adjusted
closing price. This measure captures day-to-day changes in a way that is
time-additive (daily log-returns sum to the total log-return over a week) and
mathematically symmetric (!refs)[^log-symmetry]. This symmetry makes log-returns
a convenient way to evaluate the index’s volatility and day-to-day movements.
...
Moved this down: might delete... The data were cleaned and any missing values
were removed (further data cleaning is described in \S\@ref(elementOne)). The
historical price of the index and its daily log-returns for 2018-2024 are shown
in Figure \@ref(fig:initial-visualisation).
...
of the COVID-19 pandemic was selected for exclusion (Figure
\@ref(fig:regions-to-be-removed)). The data driven procedure used to define the
COVID-19 exclusion window is described in the appendix
(\S\@ref(apd-COVID19-selection)).
...
```{r Element 6 - proportion of positive increments}
...
## Accessibility and reproducability
...
available via https://github.com/ytterbiu/a_group07-project/tree/main. Changes
made to this document were tracked using Git and are also available via the same
repository (https://github.com/ytterbiu/a_group07-project/commits/main/). Please
note that this commit history reflects updates made to the project respository
and is not a comprehensive timeline of all contributions by all group members.
