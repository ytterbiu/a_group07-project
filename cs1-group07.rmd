...
address this, we developed an interactive R Shiny dashboard, hosted via
...
define the study range and mask a specific outlier period (for example,
2020-02-24 to 2020-03-23 in the case of our report); (5) Bootstrap Settings,
allowing users to choose the number of repetitions—while 50,000 repetitions (as
used in the report) provide high precision, lower values can be selected to
reduce computation time; and (6) Analysis Tabs, which display the cleaned data,
...
resource-intensive for large numbers of repetitions.  Additionally, while the
present implementation mirrors the project’s core analysis, there is scope to
extend this to incorporate additional methods—for example, assessing normality
using bootstrapped kurtosis estimates or adding functionality to vary the m
value in the m-out-of-n bootstrap (currently hard coded as m=13).
