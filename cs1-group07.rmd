...
qualitative review, only the onset of the COVID-19 pandemic was excluded as a
non-representative outlier. Figure
...
```{r timeline-fig, echo=FALSE, fig.cap="Notable Nasdaq market events from 2018-2024. Only the period associated with COVID-19 met the numerical criteria for a systemic outlier.", out.width="80%", fig.align='center', out.width="100%"}
...
```{r skewness-densities, echo=FALSE, fig.cap = paste0("Sampling distributions of the skewness estimator for ",N_reps, " samples of size n=", n_sample_size, " taken from a Normal model (blue) and from the empirical log-returns $z_n$ via bootstrap (red).")}
...
(\S\@ref(elementOne)) and provides further evidence against an assumption
...
```{r Element 3 pt1 - TBC, echo = FALSE}
...
Kruskal–Wallis: The hypotheses are defined as:

$$
\begin{cases}
H_0: \text{The distributions of the 14 subsamples are identical} & (F_1 = F_2 = \dots = F_{14}) \\
H_1: \text{At least one subsample tends to yield larger values} & (\exists i,j: P(X_i > X_j) > 0.5)
\end{cases}
$$
OR
$$
...
H_1: \text{At least one sample comes from a distribution that tends} \\ 
\quad \ \ \text{to produce larger or smaller values than the others}
\end{cases}
$$
...
```{r nasdaqAllTimeVol, eval = TRUE, echo = FALSE, fig.height=4.4, fig.pos='!ht', fig.cap="Nasdaq Composite index price volatility from 1971-2024. The dotted line shows the volatility threshold applied to identify outliers and the red shaded region indicates the excluded window in 2018-2024 associated with the COVID-19 pandemic."}
...
deviation of daily log-returns scaled to an annual timeframe ($\sigma_{ann} =
\sigma_{daily} \times \sqrt{252}$).

As shown in Figure \@ref(fig:nasdaqAllTimeVol), this threshold acts as
a robust historical filter. Over the last 50 years, breaches of the 0.70 level
isolate only five distinct 'systemic shock' events:
...
This rarity is statistically corroborated by the fact that the 0.70 threshold
corresponds to the 
`r formatC(vol_percentiles['99.5%'], format="fg", digits=3)`th percentile of 
all trading days since 1971.

There is, therefore, strong numerical evidence to characterise the onset of the
COVID-19 pandemic as a non-representative outlier (or a period which falls
outside of the definition of 'legitimate market activity'). Other periods of
volatility in the 2018-2024 window noted in Figure \@ref(fig:timeline-fig), such
as the "Christmas Eve Massacre" (2018), the Trade War shocks (2019), and the
Inflation Bear Market (2022), did not breach this threshold. These periods fall 
into the category of legitimate and 'standard' market activity and were not 
excluded.
...
boundaries due to lag effects. Therefore, to define the exact start and end
dates of the exclusion window, technical trend line analysis was applied to the
daily price action (Figure \@ref(fig:covid-trend-lines)).
...
```{r covid-trend-lines, echo=FALSE, out.width="70%", fig.cap="Changes in the Nasdaq Composite index from x – y.  Analysis was carried out to..."}
...
tools served as a additional group member in this capacity.
...
[^cont2001quote]:
    A 'stylized empirical fact' was defined by @Cont01022001 as being
