...
To assess the general tendency of the Nasdaq index, the proportion of days with 
a positive return were analysed. The observed proportion was 
`r formatC(num_positive / total_days, format = "fg", digits = 3)` (or 
`r formatC(num_positive*100 / total_days, format = "fg", digits = 3)`%), 
suggesting a slight upward bias.

To formally assess this directional bias, an Exact Binomial Test (equivalently,
the Sign Test) was used to determine whether the median
log-return is significantly greater than zero by treating each day as a
Bernoulli trial, avoiding the normality assumptions required by a
$t$-test. While the serial dependence established in \S\@ref(elementFiv) implies
that the assumption of independent trials is an approximation, the test remains
a valid descriptive metric for the Nasdaq's general drift. The hypotheses are
defined as:
...
\end{cases}
$$
<!-- An Exact Binomial test was used to formally test whether this exceeds random  -->
<!-- chance (p=0.5). While the previous finding of serial dependence  -->
<!-- (\S\@ref(elementFiv)) implies the assumption of independent Bernoulli trials is  -->
<!-- an approximation, the test remains a valid descriptive metric for the index's  -->
<!-- directional drift. The hypotheses are defined as: -->
<!-- $$ -->
<!-- \begin{cases} -->
...
bias (towards positive returns) over 2018-2024. 

The persistence of trends was investigated by comparing sequences of
consecutive days with the same sign ('runs'). The mean length of positive runs
is
...
Given that run lengths are discrete, non-negative, and likely skewed, the
non-parametric Mann-Whitney U (Wilcoxon Rank-Sum) test was used rather than a
t-test. The Mann-Whitney U test assumes independent samples and assesses whether
the distribution of positive run lengths stochastically dominates negative run
lengths. The hypotheses are defined as:
$$
\begin{cases}
...
upward ("bullish") trends both occur more frequently and persist for more
sequential days than downward ("bearish") corrections.
...
$$
\hat{\sigma}_{ann} = \hat{\sigma}_{daily} \times \sqrt{252}
$$
